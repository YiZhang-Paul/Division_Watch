        ...
        <div class="task-list-content">
            ...
            <div class="task-node-items" :class="{ 'active-list': activeNode === 'task' }">
                <div v-for="task of tasks"
                    :key="task.name"
                    class="task-summary"
                    :style="{ 'background-color': getSummaryColor(task, true) }">

                    <div :style="{ 'background-color': getSummaryColor(task) }"></div>
                    <span>{{ task.name }}</span>
                </div>
            </div>
            ...
            <div class="interruption-node-items" :class="{ 'active-list': activeNode === 'interruption' }">
                <div v-for="interruption of interruptions"
                    :key="interruption.name"
                    class="task-summary"
                    :style="{ 'background-color': getSummaryColor(interruption, true) }">

                    <div :style="{ 'background-color': getSummaryColor(interruption) }"></div>
                    <span>{{ interruption.name }}</span>
                </div>
            </div>
...
export default class TaskList extends Vue {
    ...
    public getSummaryColor(item: TaskItem, isAlpha = false): string {
        const prefix = isAlpha ? 'rgba(' : 'rgb(';
        const suffix = isAlpha ? ', 0.5)' : ')';

        if (!item.priority) {
            return `${prefix}238, 171, 70${suffix}`;
        }

        return `${prefix}${item.priority === 1 ? '231, 72, 72' : '240, 113, 40'}${suffix}`;
    }
}
</script>
